...
function createSplash() {
  ...
  const splashUrl = isDev ? url.format({
...
function createWindow() {
  ...
  const appUrl = isDev ? "http://localhost:5173" : url.format({
...
ipcMain.handle("fetch-url", async (event, url2) => {
  const { net } = require("electron");
  return new Promise((resolve, reject) => {
    const request = net.request({ url: url2, method: "GET" });
...
ipcMain.handle("fetch-image-base64", async (event, url2) => {
  const { net } = require("electron");
  return new Promise((resolve, reject) => {
    const request = net.request({ url: url2, method: "GET" });
    request.on("response", (response) => {
